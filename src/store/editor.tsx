...
const reducerHelper = (state: EditorState, action: any) => {
  switch (action.type) {
    ...
    case 'REMOVE_HLINE':
      return {
        ...state,
        horizontalSplit: state.horizontalSplit.slice(0, Math.max(0, state.horizontalSplit.length - 1)),
      }
    case 'REMOVE_VLINE':
      return {
        ...state,
        verticalSplit: state.verticalSplit.slice(0, Math.max(0, state.verticalSplit.length - 1)),
      }
    case 'UNDO':
...
const HISTORY_AGNOSTIC_ACTIONS = ['SET_LINE_POSITION', 'UNDO', 'REDO', 'PUSH_HISTORY', 'EXPORT']
