...
const EditorSelectedGuidePanel = () => {
  ...
  const metrics = useMemo(() => {
    ...
    return null;
  }, [single, selH, selV, state.canvasSize.height, state.canvasSize.width, state.horizontalSplit, state.verticalSplit]);
  ...
  const moveToPx = (newPx: number) => {
    if (metrics && metrics.align === 'horizontal') {
      const pct = (clamp(newPx, 0, state.canvasSize.height) / (state.canvasSize.height || 1)) * 100;
      dispatch({ type: 'SET_LINE_POSITION', payload: { align: 'horizontal', index: metrics.index, position: pct } });
    } else if (metrics && metrics.align === 'vertical') {
      const pct = (clamp(newPx, 0, state.canvasSize.width) / (state.canvasSize.width || 1)) * 100;
      dispatch({ type: 'SET_LINE_POSITION', payload: { align: 'vertical', index: metrics.index, position: pct } });
    }
  };
  ...
  return (
    <div className='rounded-md border border-gray-700 p-2'>
      <div className='mb-2 flex items-center gap-2'>
        <div className='text-sm font-semibold' title='Selected guides overview'>Selection</div>
        <div className='ml-auto flex items-center gap-1'>
          ...
          <button
            className='rounded bg-gray-800 px-2 py-1 text-xs hover:bg-gray-700'
            onClick={() => dispatch({ type: 'SELECT_ONLY_AXIS_LINES', payload: { align: 'horizontal' } })}
            disabled={!state.active || state.exporting}
          ...
          <button
            className='rounded bg-gray-800 px-2 py-1 text-xs hover:bg-gray-700'
            onClick={() => dispatch({ type: 'SELECT_ONLY_AXIS_LINES', payload: { align: 'vertical' } })}
            disabled={!state.active || state.exporting}
      ...
      <div className='mb-2 text-xs text-gray-300'>Selected {selH.length + selV.length}</div>
      {single && metrics ? (
        <div className='grid grid-cols-3 items-center gap-2 text-sm'>
          <div className='col-span-1 text-gray-300'>Type</div>
          <div className='col-span-2 text-right capitalize'>{metrics.align}</div>
          <div className='col-span-1 text-gray-300'>Index</div>
          <div className='col-span-2 text-right'>{metrics.index}</div>
          <label className='col-span-1 text-gray-300' htmlFor='selectedGuidePx'>{metrics.align === 'horizontal' ? 'Y (px)' : 'X (px)'}</label>
          <div className='col-span-2'>
            <div className='flex items-center gap-2'>
              ...
              <input
                ...
                min={0}
                max={metrics.align === 'horizontal' ? state.canvasSize.height : state.canvasSize.width}
                disabled={!state.active || state.exporting}
          ...
          <div className='col-span-2 text-right'>{metrics.percent.toFixed(2)}%</div>
        </div>
      ) : null}
      <div className='mt-2 text-xs text-gray-400' title='Tips for moving and selecting guides'>
        Use Shift+Click to multi-select. Arrow keys move selected guides by {baseShift}px (hold Shift = {shiftAmount}px).
      </div>
